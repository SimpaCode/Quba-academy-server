...
exports.resolveStoredStatus = resolveStoredStatus;
exports.toDisplayStatus = toDisplayStatus;
exports.getActiveMissionId = getActiveMissionId;
exports.getOrCreateProgress = getOrCreateProgress;
exports.seedNewPlanAccess = seedNewPlanAccess;
...
// ── Runtime shim ──────────────────────────────────────────────
function resolveStoredStatus(raw) {
    if (raw === "completed")
        return "completed";
    return "not_started";
}
function toDisplayStatus(stored, isActive) {
    if (stored === "completed")
        return "completed";
    if (isActive)
        return "active";
...
function getActiveMissionId(sortedMissionIds, missionProgress) {
    for (const id of sortedMissionIds) {
        const raw = missionProgress.get(id)?.status;
        if (resolveStoredStatus(raw) === "not_started")
            return id;
    }
    return null;
}
...
const ACHIEVEMENT_RULES = [
    ...
    {
        id: "two-levels",
        check: (p) => [...p.levelStatus.values()].filter((s) => s === "completed").length >= 2,
    },
    { id: "ten-missions", check: (_, n) => n >= 10 },
    {
        id: "all-levels",
        check: (p) => [...p.levelStatus.values()].length > 0 &&
            [...p.levelStatus.values()].every((s) => s === "completed"),
    },
];
// ── Streak ────────────────────────────────────────────────────
function computeNewStreak(progress) {
    const today = (0, dateUtils_1.todayString)();
    const yesterday = (0, dateUtils_1.yesterdayString)();
...
function computeVibeScore(completedCount, streak, completedLevels) {
...
// ── getOrCreateProgress ───────────────────────────────────────
async function getOrCreateProgress(userId, unlockedPlans) {
    const userObjectId = new mongoose_1.default.Types.ObjectId(userId);
    const existing = await UserProgress_1.default.findOne({ userId: userObjectId });
    if (existing)
        return existing;
    const planFilter = unlockedPlans && unlockedPlans.length > 0
        ? { plan: { $in: unlockedPlans } }
        : {};
    const levels = await Level_1.default.find({ isPublished: true, ...planFilter }).sort({
        order: 1,
    });
    const missionProgress = new Map();
    const levelStatus = new Map();
    if (unlockedPlans && unlockedPlans.length > 0) {
        const firstLevelPerPlan = new Map();
        for (const level of levels) {
            const levelPlan = level.plan ?? "pro";
    ...
    return UserProgress_1.default.create({
...
// ── seedNewPlanAccess ─────────────────────────────────────────
async function seedNewPlanAccess(userId, newPlan) {
    const userObjectId = new mongoose_1.default.Types.ObjectId(userId);
    const progress = await UserProgress_1.default.findOne({ userId: userObjectId });
    if (!progress) {
        await getOrCreateProgress(userId, [newPlan]);
        return;
    }
    const levels = await Level_1.default.find({ isPublished: true, plan: newPlan }).sort({
        order: 1,
    });
    if (levels.length === 0)
        return;
...
async function completeMission(userId, levelSlug, missionId) {
    const [progress, level] = await Promise.all([
        getOrCreateProgress(userId),
        Level_1.default.findOne({ slug: levelSlug, isPublished: true }),
    ]);
    if (!level)
    ...
    const levelCompleted = allMissionIds.every((id) => resolveStoredStatus(progress.missionProgress.get(id)?.status) ===
        "completed");
    let nextLevelSlug = null;
    if (levelCompleted) {
        progress.levelStatus.set(levelSlug, "completed");
        const levelPlan = level.plan ?? "pro";
        const samePlanLevels = await Level_1.default.find({
            isPublished: true,
            plan: levelPlan,
        }).sort({ order: 1 });
        const currentLevelIdx = samePlanLevels.findIndex((l) => l.slug === levelSlug);
    ...
    for (const state of progress.missionProgress.values()) {
        if (resolveStoredStatus(state.status) === "completed")
            completedCount++;
    }
    const completedLevelCount = [...progress.levelStatus.values()].filter((s) => s === "completed").length;
    progress.vibeScore = computeVibeScore(completedCount, newStreak, completedLevelCount);
...
async function resetProgress(userId) {
    await UserProgress_1.default.deleteOne({
        userId: new mongoose_1.default.Types.ObjectId(userId),
    });
